...
Status ListDir(const PlatformFilename& base_dir, bool allow_not_found,
  ...
  DIR* dir = opendir(base_dir.ToNative().c_str());
  if (dir == nullptr) {
    if (allow_not_found) {
      ARROW_ASSIGN_OR_RAISE(bool exists, FileExists(base_dir));
      if (!exists) {
        return Status::OK();
      }
    }
    return IOErrorFromErrno(errno, "Cannot list directory '", base_dir.ToString(), "'");
  }
...
Result<std::vector<FileInfo>> LocalFileSystem::GetFileInfo(const FileSelector& select) {
  ...
  while (!stack.empty()) {
    ...
    auto status = ListDir(base_dir, current.allow_not_found, [&](dirent& dir_entry) {
      ...
      ARROW_ASSIGN_OR_RAISE(PlatformFilename full_fn,
                            current_base_dir.Join(dir_entry.d_name));
 ...
 private:
  ...
  class DiscoveryImplIterator {
   ...
   public:
    ...
    Status Initialize() {
      auto status =
          ListDir(dir_fn_, selector_.allow_not_found, [this](const dirent& dir_entry) {
            ARROW_ASSIGN_OR_RAISE(PlatformFilename full_fn,
                                  dir_fn_.Join(dir_entry.d_name));
